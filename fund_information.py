...
# sel_date = '2015-01-29'
fund_data_all = op_db.read("select * from test.fund_data where get_date =" + "'" + sel_date + "'")
...
def fund_information(fund_code):
...
def fund_all_information(fund_data_all_local):
    fund_all_information_local = pd.DataFrame()
    for code in fund_data_all_local.F_code:
        fund_all_information_local = fund_all_information_local.append(fund_information(code)[1:])
    return fund_all_information_local


if __name__ == "__main__":
    fund_information = fund_all_information(fund_data_all)
    fund_information['get_date'] = strftime("%Y-%m-%d", gmtime())
    op_db.save(fund_information, "fund_information")
